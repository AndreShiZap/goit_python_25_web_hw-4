    ...
    def do_GET(self):
        ...
        if route.path == '/':
            ...
            if BASE_DIR.joinpath(route.path[1:]).exists():
                self.send_static(filename=f'.{self.path}')
            else:
...
def save_data_from_form(data):
    ...
    try:
        value_dict = {key: value for key, value in [el.split('=') for el in parse_data.split('&')]}
        with open('storage/data.json', 'r', encoding='utf-8') as file:
            existing_data = json.load(file)
        existing_data[str(datetime.now())] = value_dict
...
def run_socket_server(host, port):
    ...
    try:
        ...
        logging.info("Socket server stopping...")
    finally:
        server_socket.close()
...
def storage():
    ...
    if not storage_dir.exists():
        ...
        logging.info(f"The folder '{storage_dir}' has been created")

    if not data_file.exists():
        with open(data_file, 'w') as file:
            file.write('{}')
        logging.info(f"File '{data_file}' has been created.")
